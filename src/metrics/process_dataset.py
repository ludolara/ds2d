...
class Floorplan():
    ...
    def get_unmodified_room_ids(self):
        ...
        for room in self.get_rooms():
            try:
                room_id = room["id"]
                if "Placeholder" not in str(room_id):
                    ret.add(room_id)
            except KeyError:
                pass
        return ret
    ...
    def extract_room_info(self, json_str = None):
        ...
        for room in rooms:
            ...
            for vertix in vertices:
                if 'x' in vertix.keys() and 'y' in vertix.keys():
                    polygon.append([vertix['x'],vertix['y']])
    ...
    def draw_polygons(self, rplan=False):
        ...
        for idx, id in enumerate(ids):
            ...
            plg1 = [[d['x'],d['y']] for  d in polygon.vertices]
            ...
            plg2 = [[d['x'],d['y']] for  d in polygon.sorted_vertices]
...
class FloorplansAndPrompt():
    ...
    def __init__(self, path, json_strs = None):
        self.path = path
        from src.utils.list_folders import list_json_files
        self.floorplans = {}
