...
FULL_RESULT_FOLDER=${1:-"results7_GRPO_70B_4/generations/rplan_8_70B/full_prompt"}
RESULT_FOLDER=$(echo "$FULL_RESULT_FOLDER" | cut -d'/' -f1)

echo "Using result folder: $RESULT_FOLDER"

python src/metrics_v2/numerical/run_metric.py "$FULL_RESULT_FOLDER"
...
python src/metrics_v2/diversity/run_metric.py "$RESULT_FOLDER" "direct"
